...
```
   1. После выполнения команды `npm run build`, создается папка dist в корне проекта
   2. Прокинуть содержимое папки dist в контейнер с графаной в папку /var/lib/grafana/plugins/svo-airportwidget-panel
```

   P.S. Для более подробного ознакомления запуска графаны с виджетом: [docker-compose-base](.config/docker-compose-base.yaml), [Dockerfile](.config/Dockerfile)S