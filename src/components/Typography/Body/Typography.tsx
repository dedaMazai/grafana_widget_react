...
const Typography = forwardRef<HTMLSpanElement, PropsWithChildren<TypographyProps>>(({
  ...
  return (
    <span
      ref={ref}
      {...props}
      className={classNames(typographyCls.typography, cls.body, cls[`_size-${size}`], cls[`_font-weight-${weight}`], typographyCls[`_color-${color}`], { [typographyCls._ellipsis]: ellipsis }, className)}
    >
      {children}
    </span>
  );
